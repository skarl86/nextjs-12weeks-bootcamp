...
export default function AboutPage() {
  return (
    <article className="mx-auto max-w-3xl space-y-8">
      ...
      <section>
        <h2 className="mb-3 text-xl font-bold">사전 준비</h2>
        <ul className="ml-5 list-disc space-y-1.5 text-[15px] leading-7 text-ink-700">
      ...
      <section>
        <h2 className="mb-3 text-xl font-bold">권장 학습 루틴 (주 5–7시간)</h2>
        <div className="overflow-x-auto rounded-lg border border-ink-100">
          <table className="w-full text-[14px]">
            <thead className="bg-ink-50">
              <tr>
                <th className="px-3 py-2 text-left font-semibold">요일</th>
                <th className="px-3 py-2 text-left font-semibold">활동</th>
      ...
      <section>
        <h2 className="mb-3 text-xl font-bold">캡스톤 프로젝트</h2>
        <p className="text-[15px] leading-7 text-ink-700">
          12주 동안 한 가지 앱을 점진적으로 키우길 권합니다. 매주 새로 배운
          개념을 본인의 캡스톤에 한 가지씩 녹여 넣으면, 12주 후 자연스럽게
          포트폴리오급 풀스택 앱이 완성됩니다.
        </p>
        <ul className="mt-2 ml-5 list-disc space-y-1 text-[15px] leading-7 text-ink-700">
      ...
      <section>
        <h2 className="mb-3 text-xl font-bold">진행 트래킹</h2>
        <ul className="grid gap-1.5 sm:grid-cols-2">
          {weeks.map((w) => (
            <li key={w.slug} className="text-[14px] text-ink-700">
              <Link
                href={`/curriculum/${w.slug}`}
                className="hover:text-brand-600"
              >
                <span className="mr-2 font-mono text-ink-400">
      ...
      <section>
        <h2 className="mb-3 text-xl font-bold">한눈에 보는 핵심 도구</h2>
        <ul className="ml-5 list-disc space-y-1 text-[15px] leading-7 text-ink-700">
      ...
      <section className="rounded-xl border border-ink-100 bg-ink-50 p-5 text-[15px] leading-7 text-ink-700">
        <p>
          준비됐다면{" "}
          <Link
